#!/bin/bash
# Casper
# Skrip Pencarian dan Penganalisis Google DAN Bing Dorker lengkap lengkap
# Author: CASPER
...
<-------------->
     ...
     TERKADANG JANJI HANYALAH OMONG KOSONG
...
core(){
	...
	do
		case $scan_type in
			"Single Dork Scan")
				echo
				echo "Berikan Google dork untuk digunakan: " | grep --color 'Berikan Google dork untuk digunakan'
				read DORK
				echo
			...
			"Mass Scan w/Imported Dork List")
				echo
				echo "OK, Mohon ketik Dork: " | grep --color -E 'OK||please provide path to dork file'
				read DORKFILE
				echo
				if [ ! -r "$DORKFILE" ]; then
...
bing_geo_dorker(){
	bing_banner
	echo
	echo "Welcome to Casper Dorker" | grep --color "Welcome to Casper Dorker"
	echo
...
BIZ COM INFO NET ORG AERO ASIA CAT COOP EDU GOV INT JOBS MIL MOBI MUSEUM TEL TRAVEL XXX"
	...
	echo
	injector_tester_core
	cat "$STORAGE5" > results/"$CASPERCODE".results
	sharedcnt=$(cat results/"$CASPERCODE".results | grep -v 'REGEX' | wc -l | cut -d' ' -f1)
	echo
	echo "Found $sharedcnt possibles, please check results/$CASPERCODE.results file for full details if you missed in terminal..." | grep --color -E 'Found||possibles||please check results||$CASPERCODE||results file for full details if you missed in terminal'
	echo
	rm -f GEO.txt
	banner | grep --color -E 'By||Casper'
	start_engine
